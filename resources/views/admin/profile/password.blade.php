...
<div data-aos="fade-up" data-aos-duration="1000" class="p-sm-30 p-15">
    <div class="row rg-20">
        <div class="col-xl-12">
            ...
            <form class="ajax" action="{{ route('admin.setting.profile.password.update') }}" method="POST"
                enctype="multipart/form-data" data-handler="commonResponse" autocomplete="off">
                @csrf
                <div class="p-sm-25 p-15 bd-one bd-t-zero bd-c-stroke bd-ra-8 bd-tl-ra-0 bd-tr-ra-0 bg-white mb-25">
                    <div class="row rg-20">
                        <div class="col-12">
                            <label for="editProfileCurrentPassword" class="zForm-label">{{ __('Current Password')
                                }}</label>
                            <input type="password" name="current_password" class="form-control zForm-control"
                                id="editProfileCurrentPassword" placeholder="Current Password" />
                        </div>
                        <div class="col-12">
                            <label for="editProfileNewPassword" class="zForm-label">{{ __('New Password') }}</label>
                            <input type="password" name="password" class="form-control zForm-control"
                                id="editProfileNewPassword" placeholder="{{ __('New Password') }}" />
                        </div>
                        <div class="col-12">
                            <label for="editProfileConfirmPassword" class="zForm-label">{{ __('Confirm Password')
                                }}</label>
                            <input type="password" name="password_confirmation" class="form-control zForm-control"
                                id="editProfileConfirmPassword" placeholder="{{ __('Confirm Password') }}" />
                        </div>
                    </div>
                </div>
