        ...
        <div class="row rg-20">
            <div class="col-xl-12">
                <div class="bd-one bd-c-stroke bd-ra-8 bd-bl-ra-0 bd-br-ra-0 pt-25 px-sm-25 px-10 bg-white">
                    <ul class="d-flex flex-wrap rg-20 zList-one">
                        <li>
                        ...
                        <li>
                            <a href="{{ route('admin.setting.profile.password') }}"
                ...
                <form class="ajax" action="{{ route('admin.setting.profile.update') }}" method="POST"
                    ...
                    <div class="p-sm-25 p-15 bd-one bd-t-zero bd-c-stroke bd-ra-8 bd-tl-ra-0 bd-tr-ra-0 bg-white mb-25">
                        ...
                        <div class="row rg-20">
                            <div class="col-md-6">
                                <label for="editProfileName" class="zForm-label">{{ __('Name') }}</label>
                                <input type="text" name="name" class="form-control zForm-control" id="editProfileName"
                                       value="{{ old('name', $user->name) }}" placeholder="{{ __('Name') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileNickName" class="zForm-label">{{ __('Nick Name') }}</label>
                                <input type="text" name="nick_name" class="form-control zForm-control" id="editProfileNickName"
                                       value="{{ old('nick_name', $user->nick_name) }}" placeholder="{{ __('Nick Name') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileEmail" class="zForm-label">{{ __('Email') }}</label>
                                <input type="email" name="email" class="form-control zForm-control"
                                       value="{{ old('email', $user->email) }}" id="editProfileEmail"
                                       placeholder="{{ __('Email') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileMobile" class="zForm-label">{{ __('phone_number') }}</label>
                                <input type="text" name="mobile" class="form-control zForm-control" id="editProfileMobile"
                                       value="{{ old('mobile', $user->mobile) }}" placeholder="{{ __('phone_number') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileCountry" class="zForm-label">{{ __('country') }}</label>
                                @php
                                    $profileCountry = old('country', $user->country);
                                    $profileCountryList = getCountry();
                                @endphp
                                <select name="country" id="editProfileCountry" class="form-control zForm-control sf-select-without-search">
                                    <option value="">{{ __('select') }}</option>
                                    @foreach ($profileCountryList as $countryCode => $countryName)
                                        <option value="{{ $countryCode }}" {{ $profileCountry === $countryCode ? 'selected' : '' }}>{{ $countryName }}</option>
                                    @endforeach
                                    @if ($profileCountry && ! array_key_exists($profileCountry, $profileCountryList))
                                        <option value="{{ $profileCountry }}" selected>{{ $profileCountry }}</option>
                                    @endif
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileCity" class="zForm-label">{{ __('City') }}</label>
                                <input type="text" name="city" class="form-control zForm-control" id="editProfileCity"
                                       value="{{ old('city', $user->city) }}" placeholder="{{ __('City') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileZip" class="zForm-label">{{ __('zip_code') }}</label>
                                <input type="text" name="zip_code" class="form-control zForm-control" id="editProfileZip"
                                       value="{{ old('zip_code', $user->zip_code) }}" placeholder="{{ __('Zip Code') }}"/>
                            </div>
                            <div class="col-12">
                                <label for="editProfileAddress" class="zForm-label">{{ __('Address') }}</label>
                                <textarea name="address" class="form-control zForm-control" id="editProfileAddress" rows="2"
                                          placeholder="{{ __('Address') }}">{{ old('address', $user->address) }}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="d-flex justify-content-end g-12">
                        <button
