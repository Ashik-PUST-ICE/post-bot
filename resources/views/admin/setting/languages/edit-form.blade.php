<div class="d-flex justify-content-between align-items-center bd-b-one bd-c-light-border pb-20 mb-20">
    <h5 class="fs-18 fw-600 lh-18 text-textBlack">{{ __('Update Language') }}</h5>
...
<form class="ajax reset" action="{{ route('admin.setting.languages.update', $language->id) }}" method="post"
    ...
    <div class="row rg-20 pb-20">
        <div class="col-12">
            <label for="editMultiLanguageName" class="zForm-label">{{ __('Language') }} <span
                    class="text-danger">*</span></label>
            <input type="text" id="editMultiLanguageName" class="form-control zForm-control" name="language"
        ...
        <div class="col-12">
            <label for="editAttachmentFile" class="zForm-label">{{ __('Font File') }}
                @if ($language->font)
            ...
            <input type="file" class="form-control zForm-control" id="editAttachmentFile" accept="application/pdf"
                   name="font">
            @if ($errors->has('font'))
    ...
    <div class="bd-t-one bd-c-light-border pt-17 d-flex justify-content-end g-10">
        <button type="submit"
                class="py-13 px-20 bd-one bd-ra-4 bd-c-main-color bg-main-color text-white fs-14 fw-600 lh-14">{{ __('Update')
            }}</button>
    </div>
</form>
