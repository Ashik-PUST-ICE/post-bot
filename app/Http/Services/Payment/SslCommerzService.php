...
class SslCommerzService extends BasePaymentService
{
    protected $data = [];
    protected $config = [];
    protected $apiUrl;
    ...
    private $error;
    private $apiDomain;
    private $connect_from_localhost;
    ...
    public function __construct($method, $object)
    {
        ...
        if ($this->gateway->mode == GATEWAY_MODE_SANDBOX) {
            $this->apiDomain = $this->config['apiDomainSandBox'];
            $this->connect_from_localhost = true;
        } else {
            $this->apiDomain = $this->config['apiDomainLive'];
            $this->connect_from_localhost = false;
        }

        $this->setStoreId($this->gateway->key);
        $this->setStorePassword($this->gateway->secret);
    }
    ...
    public function makePayment($amount)
    {
        ...
        $order = SubscriptionOrder::find($this->orderId);
        $requestData = array();
        ...
        $requestData['cus_city'] = '';
        $requestData['cus_state'] = '';
        $requestData['cus_postcode'] = "";
        $requestData['cus_country'] = '';
        $requestData['cus_fax'] = "";
        ...
        $requestData['product_profile'] = "digital-goods";

        # OPTIONAL PARAMETERS
        $requestData['value_a'] = "ref001";
        $requestData['value_b'] = "ref002";
        $requestData['value_c'] = "ref003";
        $requestData['value_d'] = "ref004";
        ...
        $requestData['total_amount'] = $this->amount;
        $requestData['currency'] = $this->currency;


        $header = [];

        $this->setApiUrl($this->apiDomain . $this->config['apiUrl']['make_payment']);
    ...
    public function paymentConfirmation($payment_id)
    {
        ...
        $requested_url = ($this->apiDomain . $this->config['apiUrl']['transaction_status'] . "?sessionkey=" . $payment_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");
        ...
        if ($result->status == 'VALID') {
            ...
            $data['data']['currency'] = $result->card_issuer_country_code;
            $data['data']['payment_status'] = 'success';
            $data['data']['payment_method'] = SSLCOMMERZ;
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            if ($this->SSLCOMMERZ_hash_verify($post_data, $this->getStorePassword())) {
                ...
                $requested_url = ($this->apiDomain . $this->config['apiUrl']['order_validate'] . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");

                $handle = curl_init();
                curl_setopt($handle, CURLOPT_URL, $requested_url);
                curl_setopt($handle, CURLOPT_RETURNTRANSFER, true);

                if ($this->connect_from_localhost) {
    ...
    protected function SSLCOMMERZ_hash_verify($post_data, $store_passwd = "")
    {
        if (isset($post_data) && isset($post_data['verify_sign']) && isset($post_data['verify_key'])) {
            ...
            $new_data['store_passwd'] = hash($store_passwd);
            ...
            if (hash($hash_string) == $post_data['verify_sign']) {
    ...
    {
        $this->callbackUrl = url('/') . $this->config['success_url'];
    }

    protected function setFailedUrl()
    {
        $this->callbackUrl = url('/') . $this->config['failed_url'];
    }

    protected function setCancelUrl()
    {
        $this->callbackUrl = url('/') . $this->config['cancel_url'];
    }
}
