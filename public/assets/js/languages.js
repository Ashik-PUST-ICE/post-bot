(function ($) {
    ...
    var superAdminMultiLanguageSearchTable;
    $(document).on("input", "#superAdminMultiLanguageSearch", function () {
        superAdminMultiLanguageSearchTable.search($(this).val()).draw();
    });

    superAdminMultiLanguageSearchTable = $('#commonDataTable').DataTable({
        pageLength: 25,
        ordering: false,
        serverSide: true,
        processing: true,
    ...
    $('.addmore').on('click', function (e) {
        ...
        let html = `
                    <tr>
                        <td>
                            <textarea type="text" name="key" class="key form-control zForm-control" required></textarea>
                        </td>
                        <td>
                            <input type="hidden" value="1" class="is_new">
                            <textarea type="text" name="value" class="val form-control zForm-control" required></textarea>
                        </td>
                        <td class="text-end col-1">
                            <button type="button" class="updateLangItem py-13 px-20 bd-one bd-ra-4 bd-c-main-color bg-main-color text-white fs-14 fw-600 lh-14">Update</button>
                        </td>
                    </tr>
